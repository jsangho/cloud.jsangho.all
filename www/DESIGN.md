...
tokens:
  ...
  components:
    ...
    input-search: { type: input, bg: "#1c1917", fg: "#fafafa", radius: "8px", height: "36px", padding: "0px 12px", font: "14px / 400", use: "슈퍼스타·PLE 검색 필드, inset rgba(212,175,55,0.35) 1px 링" }
    nav-link: { type: tab, fg: "#fafafa", font: "14px / 400", active: "text #d4af37", use: "상단 내비 항목 — PLE / 랭킹 / 기록 / 샵. 활성 = 메탈릭 골드" }
    card-match: { type: card, bg: "#1c1917", radius: "4px", use: "PLE 매치 카드 — 대진 중심, 그림자 없음" }
    badge-live: { type: badge, bg: "#e02020", fg: "#fafafa", radius: "4px", padding: "1px 5px", font: "12px / 400", use: "진행 중인 PLE 표시 — 유일한 비골드 시그널" }
...
KayFabe는 라이브 이벤트를 둘러싸는 화면이고, 그 디자인 언어는 하나의 일을 위해 설계된다 — 지금 벌어지는 경기와 그에 걸린 예측을, 화면 자체가 경쟁하지 않으면서 촘촘하게 훑을 수 있게 감싸는 것. 캔버스는 근흑색(`#0a0a0c`)이고 살짝 따뜻한 표면 단계(`#1c1917`)가 그 위에 얹히며, 텍스트는 순수 흰색이 아닌 근백색 잉크(`#fafafa`)에 앉아 고밀도 매치 선반을 눈부시지 않게 읽히게 한다. 시스템 전체가 단 하나의 분명한 브랜드 색을 걸친 고정보 유틸리티로 읽힌다.

그 색이 **챔피언십 골드**(`#fcbb00`)다. 이 색은 중요한 순간에만 예약된다 — **예측 제출**과 **회원가입** CTA만이 풀 pill 골드로 렌더되고, 상호작용 링크와 활성 상태는 더 깊고 금속적인 골드(`#d4af37`)로 내려가며, hover(`#f99c00`)와 pressed/서페이스(`#b75000`)가 같은 색상 램프를 따라 더 내려간다. 벨트가 금색인 이유와 같다 — 이 화면에서 금색은 "가져갈 수 있는 것"을 뜻하고, 그래서 아무 데나 칠하면 뜻이 사라진다. 골드의 모든 단계는 통제된 명도 곡선 위의 계산된 스텝이지 즉흥적인 선택이 아니다.

타이포그래피는 두 목소리로 갈린다. **Oswald**(`--font-sport`)가 브랜드·디스플레이 표면을 맡아 PLE 히어로와 챔피언십 헤드라인의 압축된 스포츠 캐스터 톤을 낸다. 제품 안쪽의 고밀도 화면은 **Pretendard**가 14px / weight 400의 일꾼 크기로 돌고, 이 플랫폼을 정의하는 고빈도 메타데이터 — 주홍색 **LIVE** 배지(`#e02020`), 픽 수, 매치 태그 — 에서 12px로 떨어진다. 지오메트리는 pill 우선이다. 버튼과 아바타는 풀 radius(9000px)이고, 기능적 크롬(검색, 배지, 카드)은 2–8px로 조인다. 엘리베이션은 사실상 없다. 깊이는 경기 카드 자체와 썸네일 위의 반투명 스크림(`rgba(0,0,0,0.6)`)에서 오지, 드롭 섀도우에서 오지 않는다.
...
- 흰 바탕의 순수 검정이 아니라, 근흑색 캔버스(`#0a0a0c`) 위의 근백색 잉크(`#fafafa`)
- 다크를 기본으로 하되 라이트 테마를 동등하게 취급하는 접근성 우선 시스템
- pill로 통일한 브랜드 크롬(9000px 버튼 + 아바타), 2–8px의 조인 기능 radius
- 그림자 없음: 깊이는 엘리베이션이 아니라 경기 카드와 반투명 스크림에서 온다
- 유일한 비골드 시그널 색으로서의 WWE 레드 `#e02020` LIVE 표시
...
### Surface & Neutral
- **Canvas** (`#0a0a0c`): 기본 페이지 배경 — 미세하게 차가운 근흑색.
- **Surface Alt** (`#1c1917`): 카드, 필드, 세그먼트 블록을 위한 살짝 더 따뜻한 표면 스텝.
...
### Signal
- **LIVE Red** (`#e02020`): 진행 중인 PLE 위의 주홍 상태 표시 — 유일한 비골드 시맨틱 색이며, 아껴서 "지금 진행 중"에만 쓴다.
- **Info Blue** (`#3b82f6`): 정보성 링크 / 인라인 안내에 쓰는 시스템 블루.
...
- Use: 진행 중인 PLE의 "LIVE" 상태 배지 — 유일한 비골드 시그널
...
- **평면 분절**: 표면은 보더나 그림자가 아니라 미묘한 배경 스텝(`#0a0a0c` → `#1c1917`)과 구조로 나뉜다.
...
- 브랜드/디스플레이는 Oswald, 고밀도 제품 UI는 Pretendard 14px / 400
- 흰 바탕 순수 검정 대신 근흑색 캔버스(`#0a0a0c`) 위 근백색 잉크(`#fafafa`)
- 시스템을 평평하게 유지한다 — 드롭 섀도우 없음, 카드 메타데이터는 반투명 스크림으로
- 버튼과 아바타는 풀 pill(9000px), 기능 크롬은 조인 2–8px radius
- WWE 레드 `#e02020`은 LIVE 표시에만 예약한다
...
- LIVE 레드를 진행 중 상태 외의 어떤 것에도 쓰지 않는다
- 세 번째 채도 높은 브랜드 색상을 섞지 않는다 — 골드와 레드가 정체성이고 그 둘로 끝이다
...
- Canvas: (`#0a0a0c`) — `bg-background`; Surface Alt: (`#1c1917`) — `bg-card`
- LIVE 시그널: (`#e02020`) — `bg-live` / `text-live`; Info: (`#3b82f6`) — `text-info`
...
6. 텍스트는 근흑색 캔버스 `#0a0a0c` 위 근백색 잉크 `#fafafa`, 흰 바탕 순수 검정이 아니다
7. WWE 레드 `#e02020`은 LIVE 전용 — 일반 액센트가 아니다
...
| State | Treatment |
|---|---|
| **Empty (예정된 PLE 없음)** | 근흑색 캔버스(`#0a0a0c`). 지금 열린 경기가 없다는 Ink(`#fafafa`) 한 줄과, 지난 기록으로 보내는 골드 CTA 하나. 무거운 일러스트 없음. |
| **Empty (검색 결과 없음)** | 결과가 없다는 Muted Stone(`#78716c`) 한 줄, 질의어를 되비추고 범위를 넓히라고 제안. 차분하고 사실적. |
| **Loading (선반 / 캐러셀)** | `#0a0a0c` 위 최종 치수의 스켈레톤 매치 카드, 4px radius, 평면 펄스 — 그림자 shimmer 없음, 그림자 없는 시스템과 일관되게. |
...
| **Skeleton** | 최종 치수의 `#1c1917` 블록, 2–4px radius, 평면 펄스. |

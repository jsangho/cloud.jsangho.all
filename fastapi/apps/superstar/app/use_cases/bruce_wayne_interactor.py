...
class BruceWayneInteractor(BruceWayneUseCase):
    """카카오 소셜 로그인(OAuth) 유스케이스 구현체.

    유저 조회/생성 로직은 ClarkKentRepository(Google 로그인 도입 시 만든 포트)를
    그대로 재사용한다 — provider를 인자로 받는 범용 인터페이스라 중복 정의가 불필요.
    """

    def __init__(
        self,
        identity_provider: KakaoIdentityProvider,
    ...
    async def login_with_kakao(self, *, code: str) -> UserModel:
        ...
        if user is not None:
            return await self._repository.link_oauth(
                user=user, provider=KAKAO_PROVIDER, oauth_id=profile.oauth_id
            )

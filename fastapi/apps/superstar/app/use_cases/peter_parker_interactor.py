...
class PeterParkerInteractor(PeterParkerUseCase):
    """네이버 소셜 로그인(OAuth) 유스케이스 구현체.
    ...
    def __init__(
        self,
        identity_provider: NaverIdentityProvider,
    ...
    async def login_with_naver(self, *, code: str) -> UserModel:
        ...
        user = await self._repository.find_by_oauth(
            provider=NAVER_PROVIDER, oauth_id=profile.oauth_id
        )
        if user is not None:
            return user
        ...
        if user is not None:
            return await self._repository.link_oauth(
                user=user, provider=NAVER_PROVIDER, oauth_id=profile.oauth_id
            )

...
4. 사용자 문구로 나가는 것은 pick·확신·요약·출처뿐이다 (모델 이름·프롬프트 비노출)

**Phase 4가 4번의 자리를 옮겼다.** 모델 이름과 프롬프트 해시는 이제 리포트에
`AgentRuntime`으로 붙어 DB까지 간다 — 막는 것은 *응답*이지 *기록*이 아니다.
그 경계와 판 식별자의 계약은 `test_agent_runtime_versions.py`가 따로 본다.

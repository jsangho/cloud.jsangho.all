...
카탈로그(`PLE_EVENT_SCHEDULE`)에 확인된 날짜가 9건 적혀 있는데도 운영 DB의
...
- **카탈로그에 없는 대회를 건드리지 않는다.** `(None, None)`을 쓰지 않는다는 뜻이다.
  일정 미정인 셋(bad-blood · king-queen-of-the-ring · survivor-series)에 누군가
  날짜를 넣어 뒀다면 그것은 카탈로그가 모르는 앎이므로, 지우지 않고 **보고만** 한다.
...
- `0` — 카탈로그 9건이 전부 DB에 있고 반영됐다(또는 이미 같았다).

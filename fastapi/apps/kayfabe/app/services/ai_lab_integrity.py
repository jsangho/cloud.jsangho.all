...
class PredictionRow:
    ...
    #: 이 예측이 가리키는 경기가 **아직 카드에 있는가** (Phase 3-13 Stage 9).
    #:
    #: 예측은 `(event_id, match_key)` 문자열로 경기를 가리킬 뿐 외래키가 아니다.
    #: 그래서 카드가 교체되면 경기 행만 사라지고 예측은 남는다 — 실제로 MITB의
    #: 가짜 카드를 진짜 대진으로 바꿨을 때 예측 한 건이 그렇게 남았다.
    #:
    #: **그 상태를 `pending`이라고 부르면 거짓이다.** 결과를 기다리는 것이 아니라

...
class PleMatchModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "match_key", name="uq_ple_event_match_key"),
    )

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    event_id: Mapped[int] = mapped_column(
        ForeignKey("ple_events.id", ondelete="CASCADE"), index=True
    )
    match_key: Mapped[str] = mapped_column(String(80), nullable=False)
    title: Mapped[str] = mapped_column(String(200), nullable=False)
    format: Mapped[str] = mapped_column(String(20), nullable=False)
    card_variant: Mapped[str] = mapped_column(
        String(10), nullable=False, default="sideA"
    )
...
class PlePredictionModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "match_id", "client_id", name="uq_ple_prediction_match_client"
        ),

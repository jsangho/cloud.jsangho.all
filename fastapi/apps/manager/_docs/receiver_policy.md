...
n8n: HTTP Request 노드 → 백엔드 API 호출 (POST 권장)
   │
   ▼
FastAPI 백엔드 (/receiver 등 수신 엔드포인트)
```
...
2. `+` → **Gmail** 검색 → 목록 하단 **Triggers** 섹션의 **On message receiver** 선택
...
4. 설정값: `Poll Times = Every Minute`, `Event = Message receiver`, `Simplify = ON`
...
기존 더미 엔드포인트 예시:
```python
@mary_mail_router.get("/receiver")
async def receiver_mail(...) -> MaryMailResponse:
    return await mary.receiver_mail(
...
- [ ] n8n Webhook 노드 (`/webhook/gmail-pubsub`) 생성

...
EVENT_POPULARITY_SCALE = 0.12
"""이벤트가 주는 인기도의 배율 (2026-08-10 · §13-Q13).

**카드의 숫자는 상대적 크기이고 절대 크기는 규칙이 정한다** — 덱 JSON은 "무엇이
일어나는가"만 적는다는 §3-D19의 연장이다.

이 상수가 없을 때 인기도 경제가 무너져 있었다. 한 판에 이벤트가 320회인데 하나가
인기도를 8~16 움직이니, 주당 ±1인 망각과 **단위가 두 자릿수 어긋났다**. 결과는
5년차 상한 도달이었고, 그 여파로 방출·부진 은퇴가 240판에서 0건이었다.

**다른 스탯에는 걸지 않는다.** 경기력·마이크웍·평판은 주차 성장도 ±1이라 이벤트와
같은 단위이고, 실제로 포화하지도 않는다. 인기도만 두 공급원의 크기가 어긋나 있었다.
"""
...
def _diminish(name: str, current: int, delta: int) -> int:
    ...
    if name == "popularity":
        # **양쪽 다 줄인다.** 이득만 줄이면 이벤트가 순 마이너스가 되어, 인기도를
        # 좇지 않는 플레이가 통째로 잊혀진다(실측에서 무작위 플레이가 13까지 내려갔다).
        # 줄이는 것은 크기이지 부호가 아니다.
        sign = 1 if delta > 0 else -1
        delta = sign * max(1, round(abs(delta) * EVENT_POPULARITY_SCALE))
    if delta <= 0:
        return delta
    if name == "alignment":

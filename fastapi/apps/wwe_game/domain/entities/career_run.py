...
class CareerRun:
    ...
    **쌓이기만 하는 값이 아니다** — 선택지가 비용을 물린다(§3-D48). 통산 수입을 따로
    ...
    goal_quarter: int = -1
    finisher: str = ""
    """지금 쓰는 피니셔의 코드 (§3-D88). **비어 있으면 계열의 기본값이다** —
    고르지 않은 것과 못 고른 것을 나누지 않는다(옛 세이브가 그 상태다)."""
    finisher_name: str = ""
    """직접 지은 기술 이름 (§3-D88). **코드 칸과 나눠 둔다** — 한 칸에 섞으면
    "모르는 코드"와 "내가 지은 이름"을 구별할 수 없다."""
    finisher_week: int = 0
    """마지막으로 손댄 주차. **기록용이다** — 언제 다시 물을지는 아래가 정한다."""
    finisher_ask_week: int = 0
    """**이 주차가 되기 전에는 안 묻는다** (§3-D88, 2026-08-14 확장).

    쿨다운 하나로는 "평생 쓰기"를 표현할 수 없어 *"언제 다시 물을 것인가"*를 직접
    담는다 — 분기·1년·평생이 전부 이 한 값이 된다(평생은 커리어 끝 너머).

    **0이면 아직 아무것도 안 정했다** — 그때는 첫 분기 규칙을 쓴다."""
    signature_slots: int = 0
    """산 시그니처 칸 수 (§3-D92). **0이면 기본값 한 칸이다** — 옛 세이브가 그 상태다.

    `finisher`가 빈 문자열을 기본값으로 읽는 것과 같은 규약이다: 마이그레이션 뒤에도
    옛 세이브의 동작이 변하지 않는다.
    """
    signature_names: tuple[str, ...] = ()
    """**돈 주고 이름 붙인 시그니처들** (§3-D92, 2026-08-19 사용자 요청).

    비어 있으면 계열 기술에서 굴려 쓴다(§3-D91) — 이름 없는 수도 경기에는 나오지만,
    그건 *내* 기술이 아니다. 한 칸을 살 때마다 경기에서 그 이름이 불릴 확률이 오른다
    (`match_flow.signature_chance`): **산 만큼 자주 나온다.**
    """
    offer_week: int = 0

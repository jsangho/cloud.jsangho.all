...
// Where DESIGN.md lives. Defaults to the repo root; set OMD_DESIGN_MD (absolute,
// or relative to the project dir) when the design system belongs to a subpackage
// instead — e.g. a monorepo where only www/ has a UI. path.resolve handles both
// forms: an absolute value ignores projectDir.
function resolveDesignMd() {
  const configured = (process.env.OMD_DESIGN_MD || '').trim();
  return configured
...
process.stdin.on('end', () => {
  ...
  const designMdPath = resolveDesignMd();
  ...
  const lines = [
    '',
    'OMD GATE',
    `⚠️  DESIGN.md not found at ${path.relative(projectDir, designMdPath) || 'DESIGN.md'}.`,

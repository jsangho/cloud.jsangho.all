...
// instead — e.g. a monorepo where only www/ has a UI. Without this the palette
// comparison reads a file that isn't there and every hex reads as off-palette.
const configuredDesignMd = (process.env.OMD_DESIGN_MD || '').trim();
const designMd = configuredDesignMd
  ? path.resolve(projectDir, configuredDesignMd)
